...
- **יועץ AI אישי (אלי):** צ'אט בעברית שרואה את הפרופיל והתיק שלך, משלב ניתוח הסוכנים, ועונה מתי לקנות/לצאת/לפזר. דורש `OPENAI_API_KEY`.
...
- `GET /api/advisor/status` — האם יועץ AI פעיל
- `POST /api/advisor/chat` — צ'אט עם יועץ AI אישי
- `POST /api/live` — מעקב חי על תיק
- `GET /api/health`
